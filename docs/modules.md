...
### `App.tsx` — 主框架
- 顶层状态：`tab` (claude/codex)、`scan` (扫描结果 + stars)、`query`、`starredOnly`、`selectedProjectKey`、`openSession`、`appearance`、`trashDir`、`settingsOpen`、`paletteOpen` / `paletteInitialView`（命令面板）。
...
- 调 `useGlobalShortcuts(handlers)` 注册全局键盘快捷键，并挂载 `<CommandPalette/>`；切 Tab 的助手 `switchTab` 同时退出回收站视图。
