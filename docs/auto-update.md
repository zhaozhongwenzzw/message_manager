...
```bash
# 升版本号、打包并发布到 GitHub Releases
npm run release:win        # 默认 patch：0.1.0 -> 0.1.1   bug fix
npm run release:win -- -p  # patch：0.1.0 -> 0.1.1   bug fix
npm run release:win -- -m  # minor：0.1.0 -> 0.2.0   新功能
npm run release:win -- -M  # major：0.1.0 -> 1.0.0   破坏性改动
```

`release:win` 会：
- 先执行对应的 `npm version`（自动改 `package.json` + git tag）
...
- **启动后**会自动后台静默检查
- 之后每小时检查一次
- 发现新版 → **弹窗提示，由用户决定**是否下载（可点「下载更新」或「跳过此版本」）
- 用户点下载 → 增量下载（只传变化的字节，靠 blockmap）
- 下载完 → 弹窗"立即重启并安装"

用户也可以随时点 Header 上的下载图标手动检查。整个流程**不会**未经确认就开始下载或安装。
